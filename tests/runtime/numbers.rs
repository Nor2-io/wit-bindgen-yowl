...
impl TestConfigurer<MyImports, Numbers> for NumbersConfigurer {
    async fn instantiate_async(&self, store: &mut Store<Wasi<MyImports>>, component: &Component, linker: &Linker<Wasi<MyImports>>) -> Result<(Numbers, Instance)>
    {
...
async fn run_test(exports: Numbers, store: &mut Store<crate::Wasi<MyImports>>) -> Result<()> {
    ...
    assert_eq!(
        exports.call_roundtrip_u8(&mut *store, u8::min_value()).await?,
        u8::min_value()
    );
    assert_eq!(
        exports.call_roundtrip_u8(&mut *store, u8::max_value()).await?,
        u8::max_value()
    );

    assert_eq!(exports.call_roundtrip_s8(&mut *store, 1).await?, 1);
    assert_eq!(
        exports.call_roundtrip_s8(&mut *store, i8::min_value()).await?,
        i8::min_value()
    );
    assert_eq!(
        exports.call_roundtrip_s8(&mut *store, i8::max_value()).await?,
        i8::max_value()
    );

    assert_eq!(exports.call_roundtrip_u16(&mut *store, 1).await?, 1);
    assert_eq!(
        exports.call_roundtrip_u16(&mut *store, u16::min_value()).await?,
        u16::min_value()
    );
    assert_eq!(
        exports.call_roundtrip_u16(&mut *store, u16::max_value()).await?,
        u16::max_value()
    );

    assert_eq!(exports.call_roundtrip_s16(&mut *store, 1).await?, 1);
    assert_eq!(
        exports.call_roundtrip_s16(&mut *store, i16::min_value()).await?,
        i16::min_value()
    );
    assert_eq!(
        exports.call_roundtrip_s16(&mut *store, i16::max_value()).await?,
        i16::max_value()
    );

    assert_eq!(exports.call_roundtrip_u32(&mut *store, 1).await?, 1);
    assert_eq!(
        exports.call_roundtrip_u32(&mut *store, u32::min_value()).await?,
        u32::min_value()
    );
    assert_eq!(
        exports.call_roundtrip_u32(&mut *store, u32::max_value()).await?,
        u32::max_value()
    );

    assert_eq!(exports.call_roundtrip_s32(&mut *store, 1).await?, 1);
    assert_eq!(
        exports.call_roundtrip_s32(&mut *store, i32::min_value()).await?,
        i32::min_value()
    );
    assert_eq!(
        exports.call_roundtrip_s32(&mut *store, i32::max_value()).await?,
        i32::max_value()
    );

    assert_eq!(exports.call_roundtrip_u64(&mut *store, 1).await?, 1);
    assert_eq!(
        exports.call_roundtrip_u64(&mut *store, u64::min_value()).await?,
        u64::min_value()
    );
    assert_eq!(
        exports.call_roundtrip_u64(&mut *store, u64::max_value()).await?,
        u64::max_value()
    );

    assert_eq!(exports.call_roundtrip_s64(&mut *store, 1).await?, 1);
    assert_eq!(
        exports.call_roundtrip_s64(&mut *store, i64::min_value()).await?,
        i64::min_value()
    );
    assert_eq!(
        exports.call_roundtrip_s64(&mut *store, i64::max_value()).await?,
        i64::max_value()
    );

    assert_eq!(exports.call_roundtrip_float32(&mut *store, 1.0).await?, 1.0);
    assert_eq!(
        exports.call_roundtrip_float32(&mut *store, f32::INFINITY).await?,
        f32::INFINITY
    );
    assert_eq!(
        exports.call_roundtrip_float32(&mut *store, f32::NEG_INFINITY).await?,
        f32::NEG_INFINITY
    );
    assert!(exports
        .call_roundtrip_float32(&mut *store, f32::NAN).await?
        .is_nan());

    assert_eq!(exports.call_roundtrip_float64(&mut *store, 1.0).await?, 1.0);
    assert_eq!(
        exports.call_roundtrip_float64(&mut *store, f64::INFINITY).await?,
        f64::INFINITY
    );
    assert_eq!(
        exports.call_roundtrip_float64(&mut *store, f64::NEG_INFINITY).await?,
        f64::NEG_INFINITY
    );
    assert!(exports
        .call_roundtrip_float64(&mut *store, f64::NAN).await?
        .is_nan());
